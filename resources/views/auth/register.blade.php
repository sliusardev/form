    ...
    <div class="min-h-screen flex items-center justify-center p-4 bg-gray-100">
        <div class="max-w-md w-full bg-white shadow-md rounded-lg p-8">
            ...
            <form method="POST" action="{{ route('auth.store') }}">
                @csrf
                <div class="mb-4">
                    <label class="block text-gray-700 mb-1" for="name">{{__('auth.name')}}</label>
                ...
                <div class="mb-6">
                    <label class="block text-gray-700 mb-1"
                           for="confirm_password">{{__('auth.password_confirmation')}}</label>
                ...
                <p class="mt-4 text-sm text-center text-gray-600">
                    {{__('auth.already_have_an_account_q')}}
                    <a href="{{route('login')}}" class="text-blue-500 hover:underline">{{__('auth.log_in')}}</a>
                </p>
            </form>
                ...
                <div class="mt-4 space-y-3">
                    <a href="{{route('socialite.auth.redirect', 'google')}}" type="button"
                       class="w-full flex items-center justify-center gap-2 border border-gray-300 rounded-lg py-2 hover:bg-gray-100 transition">
                        ...
                        <span>{{__('auth.register_with')}} Google</span>
                    </a>
                    <a href="{{route('socialite.auth.redirect', 'github')}}" type="button"
                       class="w-full flex items-center justify-center gap-2 border border-gray-300 rounded-lg py-2 hover:bg-gray-100 transition">
                        ...
                        <span>{{__('auth.register_with')}} GitHub</span>
                    </a>
                </div>
            </div>
