    ...
    <div class="tabs tabs-lift">
        ...
        <div class="tab-content bg-base-100 border-base-300 p-6">
            <form class="space-y-4" action="{{ route('company.update') }}" method="POST">
                ...
                <div>
                    <button type="submit" class="bg-gray-800 text-white px-4 py-2 rounded hover:bg-gray-700 transition cursor-pointer">{{__('dashboard.save')}}</button>
                </div>
            </form>
        </div>
        ...
        <div class="tab-content bg-base-100 border-base-300 p-6">
            <div class="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                @forelse($company->payments as $payment)
                    <div class="border rounded-md border-gray-200 p-4 bg-white flex flex-col gap-2">
                        <div class="font-semibold text-gray-700">
                            {{__('dashboard.payment_id')}}: <span class="font-normal">{{ $payment->id }} | {{ $payment->payment_id }}</span>
                        </div>
                        <div>
                            {{__('dashboard.amount')}}: <span class="font-bold text-green-600">{{ $payment->amount }} {{ $payment->currency }}</span>
                        </div>
                        <div>
                            {{__('dashboard.date')}}: <span class="text-gray-500">{{ $payment->created_at->format('Y-m-d H:m:s') }}</span>
                        </div>
                        <div>
                            {{__('dashboard.status')}}:
                            <span class="px-2 py-1 rounded text-xs font-semibold {{ $payment->getStatusColor() }}">
                                {{ $payment->status }}
                            </span>
                        </div>
                    </div>
                @empty
                    <div class="text-center text-gray-500">{{__('dashboard.no_payments')}}</div>
                @endforelse
            </div>
        </div>
    </div>
