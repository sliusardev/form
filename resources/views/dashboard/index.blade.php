...
@section('content')
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-6 mt-4">
        <div class="bg-white rounded-lg shadow p-6 text-center">
            <h4 class="text-sm lg:text-base font-medium">{{ __('dashboard.forms_count') }}</h4>
            <p class="text-2xl">{{$formsCount}}</p>
        </div>

        <div class="bg-white rounded-lg shadow p-6 text-center">
            <h4 class="text-sm lg:text-base font-medium">{{ __('dashboard.active_forms') }}</h4>
            <p class="text-2xl">{{$activeFormsCount}}</p>
        </div>

        <div class="bg-white rounded-lg shadow p-6 text-center">
            <h4 class="text-sm lg:text-base font-medium">{{ __('dashboard.total_submissions') }}</h4>
            <p class="text-2xl">{{$submissionsCount}}</p>
        </div>
        <div class="bg-white rounded-lg shadow p-6 text-center">
            <h4 class="text-sm lg:text-base font-medium">{{ __('dashboard.new_this_week') }}</h4>
